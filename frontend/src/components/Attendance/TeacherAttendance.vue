<template>
  ...
  >
    <div
      ...
      style="width: 57vw; margin-top: 3%; margin-bottom: 10px"
    >
      ...
      <template>
        <div
          class="text-right d-flex justify-content-end align-items-end"
        ...
        >
          <p
            ...
            "
          >
            {{ date.substr(0, 4) }}년 {{ date[5] + date[6] }}월
            {{ date[8] + date[9] }}일
          </p>
          ...
          >
            <template v-slot:activator="{ on, attrs }">
              <button v-bind="attrs" v-on="on">
                <img
                  src="@/assets/flaticon/calendar.png"
                  style="width: 2rem"
                  alt=""
                />
              </button>
      ...
      <div class="row row-cols-5">
        <div v-for="(kid, index) in kids" :key="kid.userId" cols="3">
          <div class="thumbnailWrap d-flex flex-column align-items-center">
            ...
            <p
              class="m-0 mt-1"
              style="text-align: center; width: 91%; font-weight: 600"
            >
              {{ kid.kidName }}
            </p>
          </div>
